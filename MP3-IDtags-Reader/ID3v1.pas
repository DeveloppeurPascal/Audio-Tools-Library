...
{ Copyright (c) 2023 by Patrick Prémartin }
{ }
{ Version 20230715 by Patrick Prémartin }
{ - upgraded to Delphi 11.3 Alexandria }
{ - removed tag updates }
{ }
...
type
  ...
  private
    ...
    function ReadTag(const FileName: WideString;
      bSetFields: boolean = true): boolean;
...
// -----------------------------------------------------------------------------------------------------------------------------------
function TID3v1.ReadTag(const FileName: WideString;
  bSetFields: boolean = true): boolean;
var
  TagData: TagRecord;
...
begin
  ...
  try
    ...
    begin
      ...
      if FLyrics2Size > 0 then
      begin
        SourceFile.Seek(SourceFile.Size - (FLyrics2Size + iOffSet),
          soFromBeginning);
        ...
        begin // is it ok ?
          ...
          begin
            ...
            begin // read all fields
              ...
              else if Field.ID = 'LYR' then
              begin
                Lyrics := Trim(String(aBuff));
                Lyrics := StringReplace(Lyrics, #13, #13#10, [rfReplaceAll]);
                Lyrics := StringReplace(Lyrics, #13#10#10, #13#10,
                  [rfReplaceAll]);
              end
              else if Field.ID = 'INF' then
              begin
                Comment := Trim(String(aBuff));
              end
              else if Field.ID = 'AUT' then
              begin
                Writer := Trim(String(aBuff));
              end
              else if Field.ID = 'EAL' then
              begin
                Album := Trim(String(aBuff));
              end
              else if Field.ID = 'EAR' then
              begin
                Artist := Trim(String(aBuff));
              end
              else if Field.ID = 'ETT' then
              begin
                Title := Trim(String(aBuff));
              end
              else if Field.ID = 'IMG' then
              begin
                FIMG := String(aBuff);
              end
              else
