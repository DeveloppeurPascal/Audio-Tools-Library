...
begin
  ...
  if FileExists(FileList.FileName) then
    ...
    if Monkey.ReadFromFile(FileList.FileName) then
      ...
      begin
        { Fill captions }
        ValidHeaderValue.Text := 'Yes, version ' + IntToStr(Monkey.Version);
        FileLengthValue.Text := IntToStr(Monkey.FileSize) + ' bytes';
        ChannelModeValue.Text := IntToStr(Monkey.Channels);
        SampleRateValue.Text := IntToStr(Monkey.SampleRate) + ' hz';
        BitsPerSampleValue.Text := IntToStr(Monkey.Bits) + ' bit';
        DurationValue.Text := FormatFloat('.000', Monkey.Duration) + ' sec.';
        FlagsValue.Text := IntToStr(Monkey.FormatFlags);
        //FramesValue.Text := IntToStr(Monkey.Frames);
        TotalSamplesValue.Text := IntToStr(Monkey.TotalSamples);
        PeakLevelValue.Text := FormatFloat('00.00', Monkey.PeakLevelRatio) + '% - ' +
          IntToStr(Monkey.PeakLevel);
        SeekElementsValue.Text := BoolToStr(Monkey.HasSeekElements);
        CompressionValue.Text := FormatFloat('0.00', Monkey.Ratio) + '% - ' +
          Monkey.CompressionModeStr;
      end
      else
        { Header not found }
