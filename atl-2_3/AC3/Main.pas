...
uses
  ...
  StdCtrls, FileCtrl, ExtCtrls, AC3;
...
begin
  ...
  if FileExists(FileList.FileName) then
    { Load AC3's Audio data }
    if AC3.ReadFromFile(FileList.FileName) then
      if AC3.Valid then
      begin
        { Fill captions }
        ValidHeaderValue.Text := 'Yes';
        FileLengthValue.Text := IntToStr(AC3.FileSize) + ' bytes';
        ChannelModeValue.Text := IntToStr(AC3.Channels);
        SampleRateValue.Text := IntToStr(AC3.SampleRate) + ' hz';
        BitsPerSampleValue.Text := IntToStr(AC3.Bits) + ' bit';
        DurationValue.Text := FormatFloat('.000', AC3.Duration) + ' sec.';
        BitrateValue.Text := IntToStr(AC3.Bitrate) + ' kbps';
        CompressionValue.Text := FormatFloat('0.00', AC3.Ratio) + '%';
      end
      else
        { Header not found }
